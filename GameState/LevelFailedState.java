...
// Shown when player dies in a level
public class LevelFailedState extends GameState{
    ...
    public LevelFailedState(StateMachine  stateMachine) {
        this.stateMachine = stateMachine;
    }

    public void init() {
        optionsStates = new int[]{stateMachine.getCurrentLevel(), StateMachine.STARTSTATE, -1};
        // Load all resources
    ...
    public void update() {
        optionsStates[0] = stateMachine.getCurrentLevel();                                                              // first buttons allows you to restart level,
                                                                                                                        // so current level is updated using getCurrentLevel() in stateMachine
    }
    public void draw(Graphics2D g) {
        ...
        g.setFont(titleFont);
        g.drawString("You Died...", 200,130);
        g.setFont(font1);
        for(int i=0; i< options.length; i++) {
            g.drawImage(uiImages[0],210,165+i*50, null);
            if(i == current) {
            ...
            g.drawString(options[i], xVals[i],200+i*50);
        }
    }
